...
export const getUserLikedMovies = createAsyncThunk("netflix/getliked",async(email)=>{
    const {data:{movies},}=await axios.get(`https://netflix-clone-3bdf.onrender.com/api/user/liked/${email}`);
    return movies;
})

export const removeFromLikedMovies = createAsyncThunk("netflix/deleteLiked",async({email,movieId})=>{
    const {data:{movies},}=await axios.put(`https://netflix-clone-3bdf.onrender.com/api/user/delete`,{email,movieId});
    return movies;
})
